...
class FuzzyNumber:
    ...
    def __init__(
        self, alphas: List[float], alpha_cuts: List[Interval], precision: int = None
    ):
        """
        ...
        if not isinstance(alphas, List):
            raise TypeError(
                "`alphas` must be a list. It is `{0}`.".format(type(alphas).__name__)
            )

        if not isinstance(alpha_cuts, List):
            raise TypeError(
                "`alpha_cuts` must be a list. It is `{0}`.".format(
                    type(alphas).__name__
                )
            )
            ...
            if not previous_interval.is_empty:
                if not (self.get_alpha_cut(alpha) in previous_interval):
                    raise ValueError(
                        ...
                        "This does not hold for {0} and {1}.".format(
    ...
    def __contains__(self, item) -> bool:
        ...
        if isinstance(item, (int, float)):
            ...
            return (
                interval.min <= item.get_alpha_cut(0).min
                and item.get_alpha_cut(0).max <= interval.max
            )
        else:
            raise TypeError(
    ...
    @staticmethod
    def get_alpha_cut_values(
        number_of_parts: int, precision: int = None
    ) -> List[float]:
        """
        ...
        if not isinstance(number_of_parts, int) or number_of_parts <= 1:
            raise ValueError(
                ...
                "It is of type `{0}` and value `{1}`.".format(
                    type(number_of_parts).__name__, number_of_parts
                )
            )
    ...
    def apply_function(
        ...
        **kwargs,
        ...
        if not isinstance(monotone, bool):
            raise ValueError(
                "`monotone` must be `bool`. `monotone` is currently `{}`.".format(
                    monotone
                )
            )
    ...
    @staticmethod
    def _iterate_alphas_one_value(
        x: FuzzyNumber, operation: Callable, *args
    ) -> FuzzyNumber:

        if not callable(operation):
            raise TypeError(
                "`operation` needs to be a function. It is `{0}`.".format(
    ...
    def _iterate_alphas_two_values(x, y, operation: Callable) -> FuzzyNumber:
        ...
        if fuzzy_x and fuzzy_y:
            alphas, intervals = FuzzyNumber.__prepare_alphas_intervals(
                x.alpha_levels, y.alpha_levels
            )
    ...
    def get_alpha_cuts_mins(
        self, alphas: List[float] = None, order_by_alphas_from_one: bool = False
    ) -> List[float]:
        """
    ...
    @staticmethod
    def _prepare_alphas(
        alpha_levels1: List[float], alpha_levels2: List[float]
    ) -> List[float]:
        """
    ...
    def trapezoidal(
        ...
        return FuzzyNumberFactory.trapezoidal(
            minimum, kernel_minimum, kernel_maximum, maximum, precision
        )

    @classmethod
    def crisp_number(cls, value: float, precision: int = None) -> FuzzyNumber:
        """
    ...
    @classmethod
    def parse_string(cls, string: str, precision: int = None) -> FuzzyNumber:
        """
