...
def fuzzy_number_triangular(min: float, kernel: float, max: float, feature: QgsFeature, parent: QgsExpression):
    if not isinstance(min, (int, float)):
        raise TypeError(f"`min` should be `int` or `float` but it is `{type(min).__name__}`.")

    if not isinstance(kernel, (int, float)):
        raise TypeError(f"`kernel` should be `int` or `float` but it is `{type(kernel).__name__}`.")

    if not isinstance(max, (int, float)):
        raise TypeError(f"`max` should be `int` or `float` but it is `{type(max).__name__}`.")
...
def fuzzy_number_trapezoidal(
    ...
    if not isinstance(kernel_min, (int, float)):
        raise TypeError(f"`kernel_min` should be `int` or `float` but it is `{type(kernel_min).__name__}`.")

    if not isinstance(kernel_max, (int, float)):
        raise TypeError(f"`kernel_max` should be `int` or `float` but it is `{type(kernel_max).__name__}`.")
